...
namespace HardcoreMode
{
    public class StatusHUD
    {
        ...
        public StatusHUD(string characterName, Vector3 position)
        {
			...
			staminaBarObject.GetComponentsInChildren<RectTransform>(true).Where(x => x.name.Contains("StaminaBar")).FirstOrDefault().sizeDelta = new Vector2(95, 25);
			...
			var foodBarObject = CreateStatusBar("FoodBar", new Vector3(90, -25, 0), statusHUD.transform);
			...
			if (healthBarGuage != null && staminaBarGuage != null && foodBarGuage != null)
				initialized = true;
		...
		public void SetVisibility(bool isVisible, bool displayHealth = true, bool displayStats = true)
        {
			if (!initialized)
				return;
		...
		public void Update(float health, float stamina, float food)
		{
